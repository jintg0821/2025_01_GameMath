...
public class TurnBasedGame : MonoBehaviour
{
    ...
    [SerializeField] float rareItemPer = 0.2f;
    [SerializeField] float currentRareItemPer;
    ...
    public TextMeshProUGUI currentRareItemPerText;
    public TextMeshProUGUI enemyCountText;

    public void StartSimulation()
    {
        // 기하분포 샘플링: 레어 아이템이 나올 때까지 반복하는 구조
        currentRareItemPer = rareItemPer;
        rareItemObtained = false;
        turn = 0;
        while (!rareItemObtained)
        {
            currentRareItemPer += 0.1f;
            SimulateTurn();
            turn++;
        }
    ...
    void SimulateTurn()
    {
        Debug.Log($"--- Turn {turn + 1} | 레어아이템 확률: {currentRareItemPer:P0} ---");

        turnText.text = $"{turn + 1}턴";
        currentRareItemPerText.text = $"레어아이템 확률 : {currentRareItemPer:P0}";

        // 푸아송 샘플링: 적 등장 수
        int enemyCount = SamplePoisson(poissonLambda);
        Debug.Log($"적 등장 : {enemyCount}");
        enemyCountText.text = $"적 등장 : {enemyCount}";
        ...
        for (int i = 0; i < enemyCount; i++)
        {
            ...
            if (totalDamage >= enemyHP)
            {
                ...
                if (reward == "Weapon" && Random.value < currentRareItemPer)
                {
                    rareItemObtained = true;
                    Debug.Log("레어 무기 획득!");
                }
                else if (reward == "Armor" && Random.value < currentRareItemPer)
                {
                    rareItemObtained = true;
